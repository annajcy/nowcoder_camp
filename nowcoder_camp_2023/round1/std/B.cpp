...
int main()
{
    ...
    { return cross(ps[v] - ps[u], ps[w] - ps[u]); };
    ...
    {
        while (j + 1 != k && area(0, j + 1, k) > area(0, j, k))
            ++j;
        ll S = area(0, j, k);
        if (S > maxArea)
            maxArea = S, s = j, t = k;
    ...
    while (true)
    {
        ...
        if (area(r, s, nxt(t)) > S)
            t = nxt(t);
        else if (area(r, s, pre(t)) > S)
            t = pre(t);
        else if (area(r, nxt(s), t) > S)
            s = nxt(s);
        else if (area(r, pre(s), t) > S)
            s = pre(s);
        else if (area(nxt(r), s, t) > S)
            r = nxt(r);
        else if (area(pre(r), s, t) > S)
            r = pre(r);
        else
            break;
    }
